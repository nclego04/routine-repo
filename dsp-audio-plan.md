...
- **Phase-checkpoint review problems (new).** At each phase checkpoint — the ✅ lines closing DSP 501–505, and the existing full-chain/full-deck days at Week 16 and Week 28 — run `review-problem-prompt.md` against 3–4 problems from at least two non-adjacent earlier weeks. Tests derivation fluency, which the cards deck doesn't. No new study days: it fills checkpoint days the plan already reserves.
...
- [`review-problem-prompt.md`](review-problem-prompt.md) — runs a short interleaved problem set at a phase checkpoint against 2+ non-adjacent earlier weeks, testing derivation fluency instead of recall. Companion to, not a replacement for, `cards.md`.
