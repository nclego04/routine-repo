...
*Capture/enhancement side (Tesla Algorithms, Rivian/VW Staff, Harman automotive):* covered cleanly. MVDR, GSC, and the multichannel Wiener filter are exactly Phase 1 (stochastic DSP) and Phase 3 (STFT), and adaptive filtering is now Phase 4's whole subject — the same invariant-core content already validated by UIC/Corey in the branch table. No new row needed: it's not a new academic specialization, it's another *industry* hiring for the same mic-array/enhancement skill set UIC already trains toward.

*Playback/rendering side (Tesla Infotainment):* not covered by Phases 0-3, and only obliquely by Phase 5. The branch table's CSUF/Faller row points partway at this — HRTF/binaural rendering is convolution-based, real-time, and spatial, structurally close to in-cabin spatial rendering, and Phase 5's partitioned convolution engine is the substrate that branch would extend. **CSUF/Faller is the one existing branch with a plausible bridge to automotive rendering work**, via shared convolution machinery, not a confirmed fit (Faller's work is headphone HRTF, not loudspeaker arrays with AudioWeaver/A2B specifics), but closer than the UIC branch that answers the capture side.

Net: automotive doesn't need its own branch-table row. The capture side is already answered by UIC/Corey; the rendering side has a weak but real bridge through CSUF/Faller that the table doesn't currently surface.
...
- **Updated for the August 2026 plan revision.** The plan now runs to Week 28 against a Fall 2027 start, not Week 16 against Spring 2027, and the two facts this check was built on both changed. Week 16 is **~April 2027** (2027-04-02) and is a *checkpoint*, not the branch point — the branch decision is now a dated event triggered by whenever the admissions answer lands, with a rule keyed to Week 22 (~2027-05-30). More importantly, **the plan now builds the portfolio artifact itself**: Phase 5 (Weeks 22–26, closing **2027-07-27**) is a real-time partitioned fast-convolution engine driving an NLMS adaptive stage on real audio, C++ with a Python reference, measured end to end.
- The original gap this check identified — "at Week 16, Phases 0-3 are done and **no portfolio artifact exists yet**" — is still true *at Week 16*, but is no longer where the plan stops. The construction that used to sit outside the plan is now Weeks 22–26 inside it.
- So the corrected read for Sonos/iZotope/Lucid: an applyable portfolio arrives around **late July 2027**, on the plan's own calendar rather than "Week 16 plus several unplanned months." That is later in absolute terms than the old estimate implied but far better specified, and it no longer depends on choosing the self-study branch — Phase 5 is branch-neutral by construction and runs in every branch.
- The branch split still matters for what happens *after*. If a program is chosen (UIC, CSUF), Sonos, iZotope, and Lucid's internship become moot as near-term targets, the person is starting graduate school, not job-hunting. **Dolby's "Recent M.S. Graduates Welcomed" framing becomes the operative rung in that branch instead, roughly two years further out at MS completion**, not the Sonos/iZotope/Lucid timeline. The difference now is that the artifact exists either way, so the no-program branch starts from a built, measured engine rather than from scratch.
...
### Portfolio currency per tier: what the postings actually reward, mapped to post-checkpoint builds
...
|---|---|---|---|---|
| No-gate rung | Sonos IC1/IC2, iZotope, Lucid internship | Working real-time code with measured improvement, over credentials | Real-time STFT-domain noise suppressor in C++, scored with PESQ/STOI on a public noisy-speech set | Largely superseded by Phase 5's engine (Weeks 22–26, all branches), which answers the same "real-time code with a measured number" test. Build this only as a second artifact, or to hit PESQ/STOI specifically |
...
| Automotive, rendering side | Tesla Infotainment, Rivian/VW Staff | Framework fluency, the reqs name JUCE, AudioWeaver, A2B, and embedded DSP targets, not theory | Wholly off-plan since the August 2026 revision removed the last JUCE affordance: a self-directed JUCE module would have to be added from scratch; AudioWeaver Designer familiarization (days-scale, graphical tool); a fixed-point port of the biquad/FIR core (~1-2 weeks); know A2B for interviews, don't build with it | Only if automotive-rendering is the target |
...
1. **The noise suppressor is the only multi-tier artifact — but the August 2026 revision moved most of its job inside the plan.** It composes almost entirely from the invariant core (Phase 3 STFT + Week 6 Wiener gain + Week 11 real-time rules + the Week 7 harness) and simultaneously answers iZotope's, Lucid's, and Sonos's named req content. That reasoning now applies to **Phase 5's convolution-plus-NLMS engine**, which draws on the same pieces, is scheduled in every branch rather than only the no-program one, and produces the same category of evidence: real-time C++ with one defensible reproducible number. The standalone noise suppressor drops from "default flagship" to "optional second artifact" — worth building if PESQ/STOI on a public speech set is specifically what a target rewards, since Phase 5's metric is ERLE or segmental SNR instead.
...
4. **The automotive split from Finding 4 carries through to artifacts: the two halves' evidence doesn't transfer.** A beamformer demo says nothing to the Infotainment/rendering reqs, and JUCE fluency says nothing to the Algorithms req. Whichever half is targeted at the branch decision determines which artifact matters; building both is the bloat the plan was rewritten to avoid.
...
- ~~Finding 4's JUCE observation sits in tension with the plan's July 2026 cut.~~ **Re-closed on different grounds, August 2026.** This was previously marked closed because `dsp-audio-plan.md`'s Week 16 table carried a conditional JUCE/plugin-framework affordance reachable via CSUF/Faller. **That is no longer true:** the August 2026 revision removed every JUCE mention from the plan — the "What got cut, and the actual reason" section is gone, and the CSUF/Faller row now reads "Phase 5's convolution engine is the substrate" rather than referencing an existing JUCE project. So the tension resolves the other way: the July 2026 cut now stands without qualification, and **automotive-rendering is off-plan by design**. Anyone targeting Tesla Infotainment or Rivian/VW Staff adds the framework work themselves; the plan will not carry them partway.

---

*Compiled from conversational research in the DSP self-study project, July 2026; cross-references updated August 2026 for the 28-week plan revision. Companion to `dsp-audio-plan.md` and the separate masters-degree research project.*
